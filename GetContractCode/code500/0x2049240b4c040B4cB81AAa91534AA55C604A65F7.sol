...
contract Lunisolar is ERC20, ERC20Burnable, Ownable, TokenRecover, Initializable {
    
    uint256 public swapThreshold;
    
    uint256 private _photonPending;
    uint256 private _liquidityPending;

    address public photonAddress;
    uint16[3] public photonFees;
    ...
    event photonAddressUpdated(address photonAddress);
    event photonFeesUpdated(uint16 buyFee, uint16 sellFee, uint16 transferFee);
    event photonFeeSent(address recipient, uint256 amount);
    ...
    constructor()
        ERC20(unicode"Lunisolar", unicode"LUNI") 
    {
        address supplyRecipient = 0xa19fdA0a2032fed9E61A0976c319327f6bE300D3;
        
        updateSwapThreshold(5000000 * (10 ** decimals()) / 10);

        photonAddressSetup(0xa19fdA0a2032fed9E61A0976c319327f6bE300D3);
        photonFeesSetup(400, 400, 0);
        ...
        _excludeFromLimits(photonAddress, true);
        ...
        _transferOwnership(0xa19fdA0a2032fed9E61A0976c319327f6bE300D3);
    }
    ...
    function getAllPending() public view returns (uint256) {
        return 0 + _photonPending + _liquidityPending;
    }

    function photonAddressSetup(address _newAddress) public onlyOwner {
        require(_newAddress != address(0), "TaxesDefaultRouterWallet: Wallet tax recipient cannot be a 0x0 address");

        photonAddress = _newAddress;
        excludeFromFees(_newAddress, true);

        emit photonAddressUpdated(_newAddress);
    }

    function photonFeesSetup(uint16 _buyFee, uint16 _sellFee, uint16 _transferFee) public onlyOwner {
        totalFees[0] = totalFees[0] - photonFees[0] + _buyFee;
        totalFees[1] = totalFees[1] - photonFees[1] + _sellFee;
        totalFees[2] = totalFees[2] - photonFees[2] + _transferFee;
        require(totalFees[0] <= 2500 && totalFees[1] <= 2500 && totalFees[2] <= 2500, "TaxesDefaultRouter: Cannot exceed max total fee of 25%");

        photonFees = [_buyFee, _sellFee, _transferFee];

        emit photonFeesUpdated(_buyFee, _sellFee, _transferFee);
    }
    ...
    function _transfer(
        ...
        if (!_swapping && !AMMPairs[from] && canSwap) {
            ...
            if (false || _photonPending > 0) {
                uint256 token2Swap = 0 + _photonPending;
                bool success = false;

                _swapTokensForCoin(token2Swap);
                uint256 coinsReceived = address(this).balance;
                
                uint256 photonPortion = coinsReceived * _photonPending / token2Swap;
                if (photonPortion > 0) {
                    success = payable(photonAddress).send(photonPortion);
                    if (success) {
                        emit photonFeeSent(photonAddress, photonPortion);
                    }
                }
                _photonPending = 0;
        ...
        if (!_swapping && amount > 0 && to != address(routerV2) && !isExcludedFromFees[from] && !isExcludedFromFees[to]) {
            ...
            if (txType < 3) {
                ...
                _photonPending += fees * photonFees[txType] / totalFees[txType];
