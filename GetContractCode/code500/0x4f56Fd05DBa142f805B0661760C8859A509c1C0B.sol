    ...
    PausableUpgradeable
{
    ...
    function mint(
        ...
        if (freeMintedPerWallet[_msgSenderERC721A()] >= 1) {
            require(
                msg.value >= mintPrice.mul(_numberOfNfts),
                "INSUFFICIENT FUNDS"
            );
    ...
    function grantBSCWLRoles(address[] memory _addresses, uint256[] memory _amounts) public onlyOwner {
        for (uint256 i = 0; i < _addresses.length; i++) {
            _grantRole(BSC_WHITELISTED_ROLE, _addresses[i]);
            claimLimit[_addresses[i]] = _amounts[i];
        }
    }
