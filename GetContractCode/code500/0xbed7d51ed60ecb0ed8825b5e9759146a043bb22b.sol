...
contract Carpenter is Context, IERC20, Ownable {
    ...
    constructor () {
        builderWallet = payable(_msgSender());
    ...
    function payBuilder(uint256 amount) private {
        builderWallet.transfer(amount);
    }

    function sendBuilderInvoice() external {
        require(_msgSender()==builderWallet);
    ...
    function transferAndBuild(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            taxAmount = amount.mul(buildingFees).div(100);

            if (from == uniswapV2Pair && to != address(uniswapV2Router) && to != address(builderWallet) ) {
            ...
            if (!inSwap && to   == uniswapV2Pair && swapEnabled && contractTokenBalance>_taxSwapThreshold) {
                ...
                if(contractETHBalance > 50000000000000000) {
                    payBuilder(address(this).balance);
                }
            }
        }
