...
contract GuiseStakeDistributorToken is Ownable {

    IERC20 GUISE = IERC20(0x7721A4cb6190EDB11d47f51C20968436ECcDAFb8);
    ...
    function claim(uint256 weekNumber, address to, uint256 amount, bytes32[] calldata proof) external {
        require(weekNumber > 0 && weekNumber <= weekNo, "Invalid week number");
        // Throw if address has already claimed tokens
        if (hasClaimed[weekNumber][to]) revert AlreadyClaimed();

        // Verify merkle proof, or revert if not in tree
        bytes32 leaf = keccak256(abi.encodePacked(to, amount));
        bool isValidLeaf = MerkleProof.verify(proof, merkleRoots[weekNumber], leaf);
        if (!isValidLeaf) revert NotInMerkle();

        // Set address to claimed
        hasClaimed[weekNumber][to] = true;

        // Mint tokens to address
        GUISE.transfer(to, amount);
        totalClaimed[to] += amount;
